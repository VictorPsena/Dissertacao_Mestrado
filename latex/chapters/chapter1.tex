...
Em geral, o objetivo da função de ativação é limitar a saída de uma unidade para um intervalo de valores, a Figura \ref{ativ_func} (a) mostra o gráfico da função sigmoid logística, enquanto a Figura \ref{ativ_func} (b) mostra o gráfico da função de limiar. 
...
\subsection{Perceptron e Algorítmo de Aprendizagem}
...
\begin{figure}[htpb]
    ...
    \caption{Estrutura básica de uma unidade e uma rede neural com duas camadas ocultas.}
    \label{fig:ex-geral}
\end{figure}
...
A Figura \ref{backprop} ilustra mostra uma rede neural simples e a equação \ref{derivadas} mostra o cálculo do gradiente para ajustar o peso da primeira camada utilizando a regra da cadeia.
...
\end{align}
em que $z_1$, $z_2$ e $z_3$ são as saídas das camadas intermediárias e $C(w,b)$ é a função de custo do erro quadrático médio, ou seja, $ C(w,b) = \frac{(\hat{y} - \sigma(z_3))^2}{n}$, $n$ sendo o número de amostras no conjunto de dados.
...
Apesar da Descida do Gradiente padrão fornecer uma direção mais precisa para o mínimo global da função de custo, o SGD é capaz de chegar próximo do mínimo de forma mais rápida e eficiente, especialmente em grandes conjuntos de dados.
Fazendo uma comparação entre os dois métodos, temos:
...
\begin{itemize}
    ...
    \item \textit{Dropout}: Durante o treinamento, essa técnica desativa aleatoriamente uma fração das unidades em cada camada da rede neural. Isso força a rede a aprender representações mais robustas e reduz a dependência de unidades específicas, ajudando a prevenir o overfitting. Em termos gerais, ao aplicar o dropout estamos treinando uma coleção de sub-redes dentro da rede original, o que promove a generalização do modelo.
    \begin{figure}[htpb]
        \centering
