...
Para exemplicar o funcionamento de como uma rede neural faz o reconhecimento de imagens, considere a tarefa de classificação de dígitos manuscritos utilizando o conjunto de dados MNIST \cite{mnist}. O conjunto de dados MNIST consiste em 70.000 imagens em escala de cinza de dígitos manuscritos (0 a 9), cada uma com uma resolução de 28x28 pixels, Figura \ref{mnist}.  
...
Cada imagem é representada como uma matriz de $28\times28$, onde cada pixel tem um valor de intensidade entre 0 (preto) e 255 (branco). Para compor a camada de entrada da rede neural, cada imagem é ``achatada" em um vetor de 784 elementos $(28\times28 = 784)$, onde cada elemento corresponde à intensidade de um pixel. O objetivo da rede neural feedforward com camdas totalmente conectadas é aprender a classificar corretamente cada imagem em uma das 10 classes correspondentes aos dígitos de 0 a 9. Como a camada de saída da rede neural deve ter 10 neurônios (um para cada classe), a arquitetura da rede pode ser definida como [784, 30, 9], onde a camada de entrada tem 784 neurônios, a camada oculta tem 30 neurônios escolhidos arbitrariamente e a camada de saída tem 9 neurônios. A função de ativação utilizada pode ser a função sigmoid logística, que é comumente usada em tarefas de classificação, com uma taxa de aprendizagem inicial de $0.1$. Treinaremos a rede ao longo de 30 épocas. Resumindo:
\

\begin{table}[h!]
    \centering
    \caption{Configurações da rede neural para classificação de dígitos manuscritos.}
    $\begin{array}{cccccc}
    \hline
    & \text{Camadas} & \text{Função de Ativação} & \text{Taxa de Aprendizado} & \text{Épocas} & \text{Acurácia} \\
    \hline
    Rede \ 1 & [784, 30, 9] & \text{Sigmoid} & 0.1 & 30 & 95\% \\
    \hline

\end{array}$
\end{table}

Para o treinamento da rede, utilizamos o algoritmo de backpropagation \ref{alg:backprop_etapas} e o Gradiente Descendente Estocástico \ref{alg:gradiente}

\section{Redes Neurais Convolucionais}
\label{redesConvolucionais}
As redes neurais convolucionais (RNCs) são uma classe especial de redes neurais projetadas para processar dados com uma estrutura de grade e que tenham uma relação de vizinhaça entre os elementos, como imagens~\cite{medium}, foram extremamentes baseadas do Cotex Visual Humano~\ref{CoViHu}. Elas são amplamente utilizadas em tarefas de visão computacional, como reconhecimento de objetos, detecção de faces e segmentação de imagens. As RNCs são inspiradas na organização do córtex visual dos animais, onde diferentes regiões do cérebro são responsáveis por processar diferentes partes do campo visual.
